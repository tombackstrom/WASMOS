...
and DCR demo tests — in the exact encrypted format the app produces, for
demonstrating/testing the analysis pipeline. This is fabricated data, not
real listener responses; item IDs and testIds match config/demo-acr.json and
config/demo-dcr.json.
...
def sample_rating(mean, listener_bias, rng):
    value = rng.gauss(mean + listener_bias, 0.5)
    return min(5, max(1, round(value)))


def generate_test(test_id, item_means, out_dir, rng):
    ...
    for i in range(1, N_LISTENERS + 1):
        ...
        responses = [
            (
                item_id,
                sample_rating(item_means[item_id], listener_bias, rng),
...
def main():
    ...
    print(f"Wrote {N_LISTENERS} synthetic result files each for ACR and DCR to {DEMO_RESULTS_DIR}")
